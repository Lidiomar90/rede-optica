...
class DB:
    ...
    def insert_staging(self, rows: list[dict], batch_id: str, usuario: str) -> int:
        ...
        cols = [
            ...
            "status_validacao","importado_por","importado_em","batch_id"
        ]
        vals = []
        for r in rows:
            vals.append((
                ...
                usuario, datetime.now(timezone.utc), r.get("batch_id","")
            ))
    ...
    def promote_batch(self, batch_id: str, usuario: str) -> int:
        ...
        if not self.cur:
            log.error("Promoção requer conexão direta (psycopg2).")
            return 0
        self.cur.execute("""
            INSERT INTO enlaces (
              id, nome, tipo_enlace, camada, status,
              ponta_a_site_id, ponta_a_site, ponta_b_site_id, ponta_b_site,
              cabo_principal_id, cabo_nome,
              distancia_m, atenuacao_ida,
              fo_ida, fo_volta, responsavel, observacao,
              origem_dado, fibras_pendentes, criticidade, batch_id,
              criado_em, atualizado_em
            )
            SELECT
              gen_random_uuid(),
              COALESCE(s.ponta_a_norm,'?') || '-' || COALESCE(s.ponta_b_norm,'?') || '-' ||
                COALESCE(s.tipo_enlace_norm,'ENL') || '-' || s.batch_id,
              COALESCE(s.tipo_enlace_norm,'outro'),
              COALESCE(s.camada_norm,'backbone'),
              'ativo',
              s.ponta_a_site_id, s.ponta_a_norm,
              s.ponta_b_site_id, s.ponta_b_norm,
              c.id,
              s.cabo_raw,
              s.distancia_m_calc, s.atenuacao_calc,
              s.fo_ida_raw, s.fo_volta_raw,
              s.responsavel_raw, s.observacao_raw,
              'telegram', true, 'media', s.batch_id,
              now(), now()
            FROM stg_enlaces_importacao s
            LEFT JOIN cabos c ON lower(c.nome) = lower(s.cabo_raw)
              AND c.inativo_em IS NULL
            WHERE s.batch_id = %s
              AND s.status_validacao = 'valido'
              AND s.promovido_em IS NULL
        """, (batch_id,))
        count = self.cur.rowcount
        self.cur.execute("""
            UPDATE stg_enlaces_importacao
            SET promovido_em = now(), promovido_por = %s, status_validacao = 'promovido'
            WHERE batch_id = %s AND status_validacao = 'valido' AND promovido_em IS NULL
        """, (usuario, batch_id))
        return count
    ...
    def rollback_batch(self, batch_id: str, usuario: str) -> int:
        ...
        self.cur.execute("""
            ...
            WHERE batch_id = %s AND inativo_em IS NULL
            RETURNING id
        """, (usuario, batch_id))
        n = self.cur.rowcount
        self.cur.execute("""
...
def processar_arquivo(
    ...
    for p in parseados:
        if p.status == "duplicata":
            ...
            if p.status in ("invalido", "duplicata", "pendente"):
                resultado.erros_detalhe.append({
                    "msg_id":  p.msg_id,
                    "status":  p.status,
                    "ponta_a": p.ponta_a_norm,
                    "ponta_b": p.ponta_b_norm,
                    "tipo":    p.tipo_enlace_norm,
                    "erros":   p.erros,
                    "raw":     p.raw_text[:200],
                })
...
def gerar_relatorio(res: ResultadoETL, caminho_saida: str):
    ...
    import csv
    csv_path = caminho_saida.replace(".txt", "_pendencias.csv")
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=["msg_id","status","ponta_a","ponta_b","tipo","erros","raw"])
        w.writeheader()
        for e in res.erros_detalhe:
            w.writerow({
                "msg_id":  e.get("msg_id",""),
                "status":  e.get("status","invalido"),
                "ponta_a": e.get("ponta_a",""),
                "ponta_b": e.get("ponta_b",""),
                "tipo":    e.get("tipo",""),
                "erros":   " | ".join(e.get("erros",[])),
                "raw":     e.get("raw","")[:200],
            })
    log.info(f"CSV de pendências: {csv_path}")
